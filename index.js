...
function extractTextInParentheses(str) {
   ...
   // Створюємо регулярний вираз для пошуку тексту в круглих дужках /\((.*?)\)/g.
   const regex = /\((.*?)\)/g;
...
function countEmails(str) {
   ...
   return Array.from(str.match(regex), (m) => m[0]).length;
}
...
console.log(
   findWordOccurrences('The quick brown fox jumps over the lazy dog. The Fox is quick.', 'fox'),
);
// Виведе  [ 16, 49 ]
...
function checkRegexMethods(regex) {
   ...
   const useMethods = [];
   if (regex.dotAll) {
      useMethods.push('dotAll');
   }

   if (regex.multiline) {
      useMethods.push('multiline');
   }

   if (regex.sticky) {
      useMethods.push('sticky');
   }
